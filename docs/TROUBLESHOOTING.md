...
wsl -d Ubuntu -e bash -lc "cd /mnt/e/Projects/Tools/LocalOCR && scripts/run_in_wsl.sh -m localocr.server"
```
...
```powershell
E:\Projects\Tools\LocalOCR\ocr_once.ps1 "E:\path\scan.pdf" -Engine vl -TimeoutSec 3600
```
...
复杂 PDF 可能被 Smart Router v2 交给 VL 子进程继续运行，普通扫描 PDF / 表单通常会走 OCR。
...
简单扫描 PDF、法律表单、送达地址确认书、空白表格和纯文字 PDF 优先使用 `auto`，让 Smart Router v2 默认走 OCR：

```powershell
E:\Projects\Tools\LocalOCR\ocr_smart.ps1 "E:\path\scan.pdf" -Engine auto
```

直接调用 `ocr_once.ps1` 时，`auto` 也会进入 API Smart Router v2。VL 更适合复杂版面、表格、公式和多栏文档，
但可能远慢于 PP-OCR。若调用方已经 124 退出且后台 VL 子进程长时间不产出，可以等待、读取已生成输出，
或用 `release_resources.ps1` / `stop_server.ps1` 清理后显式改用 `-Engine ocr`。
