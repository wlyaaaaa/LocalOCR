...
原件哈希必须一致；原件变化时不发布结果。临时快照、逐页渲染图与原子页面检查点属于该请求，退出时清理。PDF 默认 216 DPI 且限制单页渲染像素；TIFF 独立核对实际帧数。worker 中断时先读取已提交页作为部分证据，不能把一页当整份文档。
...
`job_key` 绑定源路径/hash/大小、请求语义、设备、路由策略、输出版本、模型配置、实际权重、关键依赖、实现、预处理、实际 VL 升级目标和输出目录。
...
`engine=auto` 用低成本规则选首轮模型，普通图片/扫描件先 OCR，逐页困难或表格/公式信号只升级对应页面到 VL，不按文件名猜版式；
...
解释器环境与源码快照一起发布。正式服务从所选 runtime/app 导入源码，开发仓库改动不会污染正式代码；任务、输出和检查点仍属于原项目数据路径。安装器不自动激活，验收绑定实际源码、依赖和模型。依赖、模型无关测试与禁用缓存的合成 GPU 质量测试通过且状态稳定，才能通过原子 current/previous 链接切换。回滚须有旧版本自身的有效验收，不能把目录存在当成恢复成功。

取消为不可自动重试终态。混合模型文档保留每页模型、首轮证据和坐标参照；非线性展平没有原图反向映射时明确不可映射。详见 [UPGRADING.md](UPGRADING.md)。

Linux worker 生命周期使用 PID 与 /proc 启动 ticks 识别父进程，避免系统校时影响。专属进程组中的守护子进程处理真正的协调器死亡并清理后代；不使用跟随“创建线程死亡”的 PR_SET_PDEATHSIG，避免正常 API 线程退役误杀热模型。取消、期限和真实崩溃仍清理整棵工作进程树；异常退出保留退出码诊断。
